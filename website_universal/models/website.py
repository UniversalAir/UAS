...
class Website(models.Model):
	...
	def get_branded_categories_val(self):
		cate_id = self.env['product.public.category'].search([('is_branded', '=', True)],limit=1)
		cate_ids = self.env['product.public.category'].search([('parent_id','=',cate_id.id)], order='name asc')
		return cate_ids
		...
		if sale_order and sale_order.pricelist_id:
			...
			for line in sale_order.order_line:
				if line.product_id.categ_id.id in category_dict:
					...
					price = sale_order.pricelist_id._get_product_price(line.product_id, quantity, line.product_id.uom_id, sale_order.date_order)
